...
import { OrderItemForSeller } from '../order-item-for-seller/OrderItemForSeller';

export const OrdersForSellers:FC<PropsWithChildren> = ({children}) => {
            ...
            <div className='orders-client__wrapper'>
                ...
                {orders ? 
                    <ul className='orders-client__container'>
                        {orders.map((order) => {
                            return (
                                <OrderItemForSeller role={ROLES.SELLER} toggle={toggle} orderItem={order} key={order.id_order}/>
                            )
                        })}
                    </ul> :
